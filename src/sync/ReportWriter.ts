...
export class ReportWriter {
  ...
  private renderCreatedOrUpdated(lines: string[], plan: SyncPlan): void {
    const rows = plan.buckets.filter((b): b is Extract<Bucket, { kind: 'dirty' }> => b.kind === 'dirty');
  ...
  private renderOrphans(lines: string[], plan: SyncPlan, previousContent: string): void {
    const rows = plan.buckets.filter((b): b is Extract<Bucket, { kind: 'orphan-mac' }> => b.kind === 'orphan-mac');
    if (rows.length === 0) return;

    lines.push('## Orphan cards in Mac — needs a decision');
    lines.push('');
    lines.push('_These Contacts cards have no matching note. The plugin never deletes automatically._');
    lines.push('');
    ...
    for (const b of rows) {
      ...
      lines.push(`| <span class="contact-forge-status-orphan">${cell(cardLabel(b.card))}</span> | ${actions} |`);
    }
    lines.push('');
  ...
  private renderSuggestions(lines: string[], plan: SyncPlan): void {
    const rows = plan.buckets.filter((b): b is Extract<Bucket, { kind: 'suggestion' }> => b.kind === 'suggestion');
    if (rows.length === 0) return;
  ...
  private renderErrors(lines: string[], plan: SyncPlan): void {
    const rows = plan.buckets.filter((b): b is Extract<Bucket, { kind: 'error' }> => b.kind === 'error');
    if (rows.length === 0) return;
    ...
    for (const b of rows) {
      ...
      lines.push(`| ${cell(label)} | <span class="contact-forge-status-error">${cell(b.message)}</span> |`);
    }
    lines.push('');
  }
}
